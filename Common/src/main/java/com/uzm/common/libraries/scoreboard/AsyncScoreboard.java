package com.uzm.common.libraries.scoreboard;

import com.uzm.common.libraries.scoreboard.animations.Scroller;
import com.uzm.common.reflections.MinecraftReflections;
...
public class AsyncScoreboard {
    ...
    static {
        try {
            if (MinecraftReflections.nmsOptionalClass("ScoreboardServer$Action").isPresent()) {
                VERSION_TYPE = VersionType.V1_13;
            } else if (MinecraftReflections.nmsOptionalClass("IScoreboardCriteria$EnumScoreboardHealthDisplay").isPresent()) {
                VERSION_TYPE = VersionType.V1_8;
            } else {
                VERSION_TYPE = VersionType.V1_7;
            }

            Class<?> craftChatMessageClass = MinecraftReflections.obcClass("util.CraftChatMessage");
            Class<?> entityPlayerClass = MinecraftReflections.nmsClass("EntityPlayer");
            Class<?> playerConnectionClass = MinecraftReflections.nmsClass("PlayerConnection");
            Class<?> craftPlayerClass = MinecraftReflections.obcClass("entity.CraftPlayer");

            MESSAGE_FROM_STRING = craftChatMessageClass.getDeclaredMethod("fromString", String.class);
            CHAT_COMPONENT_CLASS = MinecraftReflections.nmsClass("IChatBaseComponent");
            ...
            SEND_PACKET = playerConnectionClass.getDeclaredMethod("sendPacket", MinecraftReflections.nmsClass("Packet"));

            PACKET_SB_OBJ = MinecraftReflections.nmsClass("PacketPlayOutScoreboardObjective").getConstructor();
            PACKET_SB_DISPLAY_OBJ = MinecraftReflections.nmsClass("PacketPlayOutScoreboardDisplayObjective").getConstructor();
            PACKET_SB_SCORE = MinecraftReflections.nmsClass("PacketPlayOutScoreboardScore").getConstructor();
            PACKET_SB_TEAM = MinecraftReflections.nmsClass("PacketPlayOutScoreboardTeam").getConstructor();

            if (VersionType.V1_8.isHigherOrEqual()) {
                ENUM_SB_HEALTH_DISPLAY = MinecraftReflections.nmsClass("IScoreboardCriteria$EnumScoreboardHealthDisplay");

                if (VersionType.V1_13.isHigherOrEqual()) {
                    ENUM_SB_ACTION = MinecraftReflections.nmsClass("ScoreboardServer$Action");
                } else {
                    ENUM_SB_ACTION = MinecraftReflections.nmsClass("PacketPlayOutScoreboardScore$EnumScoreboardAction");
                }

                ENUM_SB_HEALTH_DISPLAY_INTEGER = MinecraftReflections.enumValueOf(ENUM_SB_HEALTH_DISPLAY, "INTEGER");
                ENUM_SB_ACTION_CHANGE = MinecraftReflections.enumValueOf(ENUM_SB_ACTION, "CHANGE");
                ENUM_SB_ACTION_REMOVE = MinecraftReflections.enumValueOf(ENUM_SB_ACTION, "REMOVE");
            } else {
    ...
    public AsyncScoreboard(Player player, Scroller scroller) {
    ...
    public void updateLines(String... lines) {
        ...
        l.replaceAll(c -> ChatColor.translateAlternateColorCodes('&', c));
        updateLines(l);
    }
