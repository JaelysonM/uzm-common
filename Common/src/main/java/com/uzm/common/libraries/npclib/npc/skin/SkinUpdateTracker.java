  ...
  private class PlayerTracker {
    ...
    void reset(Player player) {
      ...
      if (rotationCount < 3) {
        ...
        float yaw = MathUtils.clampYaw(this.location.getYaw());
        this.startYaw = yaw;
        this.upperBound = MathUtils.clampYaw(yaw + rotationDegrees);
        this.lowerBound = MathUtils.clampYaw(yaw - rotationDegrees);
    ...
    boolean shouldUpdate(Player player) {
      ...
      if (rotationCount < 3) {
        float yaw = MathUtils.clampYaw(currentLoc.getYaw());
        boolean hasRotated;
